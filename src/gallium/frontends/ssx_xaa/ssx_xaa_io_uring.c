/*
 * SonicMesa - ssX XAA io_uring Implementation
...
struct ssx_xaa_ring_ctx *ssx_xaa_ring_init(uint32_t ring_size, uint32_t cmd_buf_size)
{
    ...
    /* Setup io_uring params */
    params.flags = IORING_SETUP_SQPOLL;
    params.flags |= IORING_SETUP_SQ_AFF;  /* Fixed buffer mode */
    params.sq_thread_idle = 1000;  /* 1 second idle before sleep */
    
    /* Create io_uring ring */
    ctx->ring_fd = io_uring_queue_init(ring_size, &params, 0);
    if (ctx->ring_fd < 0) {
        fprintf(stderr, "ssX XAA: io_uring_queue_init failed: %d\n", -ctx->ring_fd);
        free(ctx);
        return NULL;
    }
                       ...
                       uint32_t cmd_size, uint32_t flags)
{
    ...
    /* Prepare SQE for write */
    io_uring_sqe_set_data(sqe, (void *)(uintptr_t)slot);
    
    /* Use write with fixed buffer for zero-copy */
    io_uring_prep_write_fixed(sqe, ctx->ring_fd,
                              ctx->cmd_buffer + (slot * 64),
                              cmd_size, 0, slot);
                                            ...
                                            uint64_t timeout_ns)
{
    ...
    if (timeout_ns > 0) {
        ret = io_uring_wait_cqe_timeout(ctx->ring_fd, &cqe, 
                                         (struct __kernel_timespec *)timeout_ns);
    } else {
